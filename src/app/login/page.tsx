...
const LoginPage  = () => {
  ...
  const onLogin = async () => {
    try {
      ...
      toast.success("Login successful 🥳",    {duration: 9000});
      router.push("/profile")
    } catch (error: any) {
  ...
  reverseOrder={false}
/>
    <h1 className='text-center text-black text-3xl'>{loading ? "Proccessing" : "Login"}</h1>
    <hr />
    ...
    <button onClick={onLogin} className="p-2 border-gray-300 rounded-lg mb-4 focus:border-gray-600">{buttonDisabled ? "No Login" : "Login"}</button>
